...
def get_course_run_data(course_run_id, fields):
    """
    Retrieve information about the course with the given course_run_id.

    Arguments:
        course_run_id: id for the course run about which we are retrieving information.
        fields (List, string): The given fields that you want to retrieve from API response.

    Returns:
        dict with details about specified course run id.
    """
    user, catalog_integration = check_catalog_integration_and_get_user(error_message_field='Course Run UUID')
    if user:
        api_client = get_catalog_api_client(user)
        base_api_url = get_catalog_api_base_url()
        if course_run_id:
            course_cache_key = f'{catalog_integration.CACHE_KEY}.course.{course_run_id}'
            data = get_api_data(
                catalog_integration,
                'course_runs',
